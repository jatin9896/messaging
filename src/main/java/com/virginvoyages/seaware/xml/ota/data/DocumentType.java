...
/**
 * Update mechanism for Visas: Match existing record by Number. If found, update that record. If not, populate into any record (Visa or Secondary Visa). Note that Seaware APIS form only supports two visa records, so request must not contain moew than two visa records (visa records are document elements with DocType 1 or 14).
 * 
 ...
 *       &lt;attribute name="DocIssueStateProv" type="{http://www.versonix.com/ota}StateProvCodeTypeExtensionType" />
...
@XmlSeeAlso({
    com.virginvoyages.seaware.xml.ota.data.AirTravelerType.Document.class
})
